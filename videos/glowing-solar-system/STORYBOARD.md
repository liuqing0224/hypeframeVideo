---
format: 1920x1080
message: "像一场会发光的博物馆宇宙探险。"
arc: Start -> Challenge -> Resolution
audience: primary-and-middle-school-students
---

## Frame 1 - 熄灭的展厅

- status: animated
- src: compositions/01-start.html
- duration: 13.000s
- transition_in: cut
- scene: 中央悬浮太阳系轨道装置和环形展台；穿橙色连帽外套的中国女孩小曦，抬头看着熄灭的太阳系模型
- voiceover: 旁白：中央太阳系模型正在一颗颗熄灭。 / 小曦：阿朗，你看，行星正在失去亮光！ / 小光：能量连接异常，请帮助我修复轨道。
- motion: multi-phase-camera, waterfall-entry, sine-wave-loop
- sfx: shot-synced impact, cut, and rustle cues

熄灭的展厅承担故事的 start 节点。对白、镜头和声效按三个内部镜头对齐。

### Shot 1.1 - establish

- from: 0.000s
- duration: 4.566s
- framing: wide
- focus: ensemble

### Shot 1.2 - discovery

- from: 4.566s
- duration: 4.044s
- framing: medium
- focus: primary

### Shot 1.3 - reaction

- from: 8.610s
- duration: 4.390s
- framing: close
- focus: primary

## Frame 2 - 追逐能量

- status: animated
- src: compositions/02-middle.html
- duration: 14.000s
- transition_in: chromatic-split
- scene: 故障中的控制环、能量核心和透明全息支架；小曦向前奔跑并伸手抓住一块金色能量碎片
- voiceover: 旁白：三人沿着行星轨道寻找能量碎片。 / 阿朗：先按顺序连接轨道，别接错位置！ / 小曦：我去拿金色碎片，小光负责扫描！
- motion: coordinate-target-zoom, motion-blur-streak, reactive-displacement
- sfx: shot-synced impact, cut, and rustle cues

追逐能量承担故事的 middle 节点。对白、镜头和声效按三个内部镜头对齐。

### Shot 2.1 - pressure

- from: 0.000s
- duration: 4.782s
- framing: wide
- focus: ensemble

### Shot 2.2 - action

- from: 4.782s
- duration: 4.188s
- framing: medium
- focus: primary

### Shot 2.3 - decision

- from: 8.970s
- duration: 5.030s
- framing: close
- focus: primary

## Frame 3 - 点亮未知

- status: animated
- src: compositions/03-end.html
- duration: 11.500s
- transition_in: grid-dissolve
- scene: 恢复运转的金色太阳核心和完整环形轨道；小曦站在中央高举最后一块能量碎片，脸上充满惊喜
- voiceover: 旁白：太阳和八颗行星全部重新发光。 / 阿朗：最后一条轨道接通了！ / 小光：好奇心，就是点亮未知的光。
- motion: center-outward-expansion, ambient-glow-bloom, particle-burst
- sfx: shot-synced impact, cut, and rustle cues

点亮未知承担故事的 end 节点。对白、镜头和声效按三个内部镜头对齐。

### Shot 3.1 - climax

- from: 0.000s
- duration: 4.614s
- framing: medium
- focus: primary

### Shot 3.2 - payoff

- from: 4.614s
- duration: 2.868s
- framing: close
- focus: primary

### Shot 3.3 - resolution

- from: 7.482s
- duration: 4.018s
- framing: wide
- focus: ensemble
